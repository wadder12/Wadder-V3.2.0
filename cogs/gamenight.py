...
class GameNight(commands.Cog):
    ...
    @commands.command(name="create-gamenight")
    async def create_game_night(self, ctx, name: str, date: str, time: str):
        ...
        await self.update_game_night_embed(ctx, event_id)

    @commands.command(name="list-gamenights")
    async def list_game_nights(self, ctx):
        if not self.game_nights:
            await ctx.send("There are no game nights scheduled.")
            return
    ...
    @commands.command(name="cancel-gamenight")
    async def cancel_game_night(self, ctx, event_id: str):
        if event_id not in self.game_nights:
            await ctx.send("Invalid event ID.")
            return

        del self.game_nights[event_id]
        await ctx.send("Game night cancelled.")
