...
class MarketNews(commands.Cog):
    ...
    async def set_news_channel(self, ctx, channel: nextcord.TextChannel):
        self.channel_id = channel.id
        await ctx.send(f"Market news channel set to {channel.mention}.")

    @tasks.loop(minutes=1)  # Update the news every 60 minutes
